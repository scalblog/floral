...
<html lang="fr">
  ...
  <body>
    <main>
    <h2>Liens Google Maps des coiffeurs à Paris</h2>
    ...
    $coiffures = json_decode(file_get_contents('coiffeursparis.json'));
    // var_dump ($coiffures);
    ?>
    <p id="75001"><b>75001</b></p>
    <?php
    foreach ($coiffures as $key => $coiffure) {
          
        
        if($coiffure->fields->arro == 75001) {
        ?>
            <ul>
                <li>index n°<?= $key ?> <?= $coiffure->fields->arro ?> : <a href="https://www.google.fr/maps/search/coiffure+<?= $coiffure->fields->adresse_complete?>+<?= $coiffure->fields->arro ?>/" target="_blank"><?= $coiffure->fields->adresse_complete ?></a></li>
            </ul>
        <?php    }
        }
