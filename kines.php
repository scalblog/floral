...
<html lang="fr">
  ...
  <body>
    <main>
    <h2>Liens Google Maps des kinés à Paris</h2>
    ...
    $kinés = json_decode(file_get_contents('kinesparis.json'));
    // var_dump ($kinés);
    ?>
    <p id="75001"><b>75001</b></p>
    <?php
    foreach ($kinés as $key => $kiné) {
          
        
        if($kiné->fields->arro == 75001) {
        ?>
            <ul>
                <li>index n°<?= $key ?> <?= $kiné->fields->arro ?> : <a href="https://www.google.fr/maps/search/kiné+<?= $kiné->fields->adresse_complete?>+<?= $kiné->fields->arro ?>/" target="_blank"><?= $kiné->fields->adresse_complete ?></a></li>
            </ul>
        <?php    }
        }
