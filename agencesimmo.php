...
<html lang="fr">
  <head>
    <title>Agences immobilières à Paris</title>
    <!-- Required meta tags -->
    <meta charset="utf-8">
  ...
  <body>
    <main>
    <h2>Liens Google Maps des agences immobilières à Paris</h2>
    ...
    $agences = json_decode(file_get_contents('agencesimmoparis.json'));
    // var_dump ($agences);
    ?>
    ...
    foreach ($agences as $key => $agence) {
        ...
        if($agence->fields->arro == 75001) {
        ?>
            <ul>
                <li>index n°<?= $key ?> <?= $agence->fields->arro ?> : <a href="https://www.google.fr/maps/search/agence+<?= $agence->fields->adresse_complete?>+<?= $agence->fields->arro ?>/" target="_blank"><?= $agence->fields->adresse_complete ?></a></li>
            </ul>
        <?php    }
        }
