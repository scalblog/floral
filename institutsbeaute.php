...
<html lang="fr">
  ...
  <body>
    <main>
    <h2>Liens Google Maps des instituts à Paris</h2>
    ...
    $instituts = json_decode(file_get_contents('institutsbeauteparis.json'));
    // var_dump ($instituts);
    ?>
    <p id="75001"><b>75001</b></p>
    <?php
    foreach ($instituts as $key => $institut) {
          
        
        if($institut->fields->arro == 75001) {
        ?>
            <ul>
                <li>index n°<?= $key ?> <?= $institut->fields->arro ?> : <a href="https://www.google.fr/maps/search/institut+<?= $institut->fields->adresse_complete?>+<?= $institut->fields->arro ?>/" target="_blank"><?= $institut->fields->adresse_complete ?></a></li>
            </ul>
        <?php    }
        }
